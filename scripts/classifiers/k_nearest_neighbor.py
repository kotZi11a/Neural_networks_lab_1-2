...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        # Квадраты норм тестовых точек (sum по оси признаков)
        test_sq = np.sum(X ** 2, axis=1)
        # Квадраты норм тренировочных точек
        train_sq = np.sum(self.X_train ** 2, axis=1)
        # Матрица попарных скалярных произведений
        dot_product = np.dot(X, self.X_train.T)
        # Вычисляем квадраты расстояний через broadcasting
        dists = np.sqrt(test_sq[:, np.newaxis] + train_sq - 2 * dot_product)
        self.dists = dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # Сортируем индексы по расстоянию (от ближайшего к дальнему)
            sorted_indices = np.argsort(dists[i, :])
            # Берем первые k индексов
            ...
            # Выбираем все метки с максимальной частотой
            candidates = labels[counts == max_count]
            # Берем наименьшую метку в случае ничьи
